...
// onboarding when it is still pending — deciding here would use a stale closure.
  ...
  return <Screen><BrandMark withName /><PageHeader title="Que bom ter você de volta." subtitle="Entre para continuar de onde parou." /><View style={uiStyles.form}>
    ...
    <Link href="/auth/forgot" style={uiStyles.link}>Esqueci minha senha</Link>{message ? <Text accessibilityLiveRegion="polite" style={styles.message}>{message}</Text> : null}<Button label="Entrar" loading={busy} onPress={submit} /><Button label="Continuar com Google" variant="secondary" loading={busy} onPress={submitGoogle} />
