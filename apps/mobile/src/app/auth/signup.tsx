...
export default function SignupScreen() {
  ...
  async function submit() { const next = validateSignUp(input); setErrors(next); setMessage(''); if (Object.keys(next).length) return; setBusy(true); try { await signUp(input); router.push('/auth/verify'); } catch (error) { setMessage(error instanceof Error ? error.message : 'Não foi possível criar a conta.'); } finally { setBusy(false); } }
