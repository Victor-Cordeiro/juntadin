...
export default function SignupScreen() {
  ...
  async function submitGoogle() { setBusy(true); setMessage(''); try { await signInWithGoogle(); router.replace('/dashboard'); } catch (error) { setMessage(error instanceof Error ? error.message : 'Não foi possível continuar com Google.'); } finally { setBusy(false); } }
  return <Screen><BrandMark withName /><PageHeader eyebrow="60 dias sem cartão" title="Crie sua conta." subtitle="Seu histórico continua seu, mesmo se o teste terminar." /><View style={uiStyles.form}>
    ...
    {message ? <Text style={styles.message}>{message}</Text> : null}<Button label="Criar conta" loading={busy} onPress={submit} /><Button label="Continuar com Google" variant="secondary" loading={busy} onPress={submitGoogle} /><View style={styles.login}><Text style={uiStyles.body}>Já tem conta?</Text><Link href="/auth/login" style={uiStyles.link}> Entrar</Link></View>
  </View></Screen>;
}
