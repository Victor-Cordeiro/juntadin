...
export default function NewCategoryScreen() {
  const router = useRouter(); const goBack = useGoBack('/settings/categories'); const nameRef = useAutoFocus(); const params = useLocalSearchParams<{ kind?: CategoryKind }>(); const kind: CategoryKind = params.kind === 'income' ? 'income' : 'expense'; const { addCustomCategory } = usePrototype();
  ...
  return <Screen>
    <View style={styles.top}><Pressable accessibilityLabel="Voltar" onPress={goBack}><Text style={styles.back}>‹</Text></Pressable><Text accessibilityRole="header" style={styles.title}>Nova categoria</Text><Pressable accessibilityLabel="Salvar categoria" onPress={save}><Text style={styles.save}>Salvar</Text></Pressable></View>
    <View style={styles.preview}><View style={[styles.previewIcon, { backgroundColor: `${color}22` }]}><CategoryIcon name={icon} color={color} size={42} /></View><Text style={styles.previewName}>{name || 'Nome'}</Text><Text style={styles.counter}>{name.length}/30</Text></View>
    <Field ref={nameRef} label="Nome da categoria" value={name} onChangeText={setName} placeholder={kind === 'expense' ? 'Ex.: Pets' : 'Ex.: Comissão'} />
